...
function DanceSong(name,singer,songUrl) {
    this.name = name;
    this.singer = singer;
...
PageDance.prototype = {
    ...
    showSong:function () {
        var owner = this;
        var songNull = "";
        if(this.danceSongS[owner.optSongIndex].songUrl == null || this.danceSongS[owner.optSongIndex].songUrl == undefined || this.danceSongS[owner.optSongIndex].songUrl == songNull){
            console.log("json中没有songUrl")
        }else{
            var audios = this.danceSongS[this.optSongIndex];
            ...
            audio.addEventListener('ended', function () {
                ...
                owner.optSongIndex++;
                audios = owner.danceSongS[owner.optSongIndex];
                $("#audio").attr("src", audios.songUrl);
                audio.play();
                audio.loop = false;
            }, false);
...
var itemCanvas = new PageDance();

function readJson() {
    $.getJSON("json/danceDataS.json", function (data) {
        var songInfos = [];
...
function readDanceSong() {
    $.getJSON("json/danceSong.json",function (data) {
        var songInfos = [];
        ...
        $.each(data, function (i, data) {
            ...
            var info = new DanceSong(name,singer,songUrl);
            songInfos.push(info);
        });

        itemCanvas.danceSongS = songInfos;
        itemCanvas.start();
    })
}
