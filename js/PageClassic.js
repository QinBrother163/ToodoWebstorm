...
function ClassicSong(name,singer,songUrl) {
    this.name = name;
    this.singer = singer;
...
PageClassic.prototype = {
    ...
    showSong:function () {
        var owner = this;
        var songNull = "";
        if(this.classSongS[owner.optSongIndex].songUrl == null || this.classSongS[owner.optSongIndex].songUrl == undefined || this.classSongS[owner.optSongIndex].songUrl == songNull){
            console.log("json中没有songUrl")
        }else{
            var audios = this.classSongS[this.optSongIndex];
            ...
            audio.addEventListener('ended', function () {
                ...
                owner.optSongIndex++;
                audios = owner.classSongS[owner.optSongIndex];
                $("#audio").attr("src", audios.songUrl);
                audio.play();
                audio.loop = false;
            }, false);
...
var itemCanvas = new PageClassic();

function readJson() {
    $.getJSON("json/classicDataS.json", function (data) {
        var songInfos = [];
...
readJson();
function readClassicSong() {
    $.getJSON("json/classicSong.json",function (data) {
        var songInfos = [];
        ...
        $.each(data, function (i, data) {
            ...
            var info = new ClassicSong(name,singer,songUrl);
            songInfos.push(info);
        });

        itemCanvas.classSongS = songInfos;
        itemCanvas.start();
    })
}